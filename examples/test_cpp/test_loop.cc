...
void test_loop::run(bool is_ccb) {
  ...
  for (size_t i = 0; i < threads; ++i) {
    _threads.push_back(is_perf ? (is_ccb ? std::thread(&test_loop::perf_ccb_loop, this, i) : std::thread(&test_loop::perf_cb_loop, this, i)) : std::thread(&test_loop::validity_loop, this, i));
  }
  for (size_t i = 0; i < threads; ++i) {
    _threads[i].join();
  }
...
void test_loop::perf_cb_loop(size_t thread_id)
{
  r::ranking_response response;
  r::api_status status;
...
void test_loop::perf_ccb_loop(size_t thread_id)
{
	...
	size_t choose_rank_size = 0;
	{
		...
		auto decision_event = r::decision_ranking_event::request_decision(event_ids_c, context.c_str(), r::action_flags::DEFAULT, response, timestamp{});
