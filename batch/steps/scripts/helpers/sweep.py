...
def _output(candidates, grid_config, env):
    return list(map(lambda item: command.labeled(grid_config.name + str(item[0]), item[1][0]), enumerate(_top(candidates, grid_config.output))))
...
def _iteration(grid, env):
    ...
    for c in candidates:
        env.logger.info(str(command.to_commandline(c[0])) + ': ' + str(c[1]))
